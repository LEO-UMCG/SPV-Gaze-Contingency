...
from torchvision.models import feature_extraction as extractor
...
def get_args(args_list=None):
    ...
    ap.add_argument("-bs", "--binned_stimulation", type=str2bool, default=False,
    ...
    # ap.add_argument("-kr", "--kappa_repr", type=float, default=0.01,
    #                 help="representation weight parameter kappa") 
    ap.add_argument("-ks", "--kappa_spars", type=float, default=0.01,
                    help="sparsity weight parameter kappa")    
    ap.add_argument("-j", "--j_exp_no", type=float, default=0,
                    help="0=not jaap, 3 or 4 based on experiment setting")
...
def full_fig2(*tensors,title=None,classes=None): #img_tensor, phs_tensor, recon_tensor    

    fig = plt.figure(figsize=(60, 60))
    
    for i,t in enumerate(tensors[0]):
        ...
        # #added
        # ax = fig.add_subplot(img.shape[0],img.shape[1])
        # #added
        # for k,j in zip(range(img.shape[0]), range(img.shape[1])):
        #     im = ax.imshow(img[k][j])
          
        # for j,k in zip(range(img.shape[0]), range(img.shape[1])):
            # ax = fig.add_subplot(len(tensors[0]),img.shape[0],i*img.shape[0]+(j+1))
            # ax = fig.add_subplot(len(tensors[0]),img.shape[1],i*img.shape[0]+(j+1))

        # for j in range(img.shape[0]):
        #   for k in range(img.shape[1]):
        #     ax = fig.add_subplot(len(tensors[0]),img.shape[0]*img.shape[1],i*img.shape[0]+(j+1))
            # ax = fig.add_subplot(img.shape[0], img.shape[1],j*img.shape[1]+(k+1))
        
        for k in range(img.shape[1]):
            ax = fig.add_subplot(len(tensors[0]),img.shape[1],i*img.shape[1]+(k+1))
            ...
            #     im = ax.imshow(img[j][1],cmap='gray',vmin=0,vmax=1)
            # else:
            # im = ax.imshow(img[j,k,:,:] ,cmap='gray') #.transpose(1,2,0))
            im = ax.imshow(img[0,k,:,:] ,cmap='gray')
            ax.axis('off')
            # divider = make_axes_locatable(ax)
            # cax = divider.append_axes("right", size="5%", pad=0.05)
            # plt.colorbar(im, cax=cax)
...
def full_fig3(model,samples):  

    samples_initial = samples
    ...
    for layer in list(model.children())[-1]:
        ...
        if len(list(layer.children()))!=0:
          ...
          for layer in list(layer.children()):
            out = layer(samples)
            samples=out
    ...
    for i in range(samples.shape[0]):
      ax = fig.add_subplot(len(figs)+1,samples.shape[0],i+1)
      im = ax.imshow(samples_initial.detach().cpu().numpy()[i][0] ,cmap='gray')
      ax.axis('off')

    for j in range(len(figs)):
      for k in range(samples.shape[0]):
        ax = fig.add_subplot(len(figs)+1,samples.shape[0],(j+1)*samples.shape[0]+(k+1))
        im = ax.imshow(figs[j].detach().cpu().numpy()[k][0] ,cmap='gray')
        # plt.imshow(figs[j][k][0].detach().cpu().numpy() ,cmap='gray')
        # plt.show()
        ax.axis('off')
...
def log_gradients_in_model(model, model_name, logger, step):
    ...
    for tag, value in model.named_parameters():
        if value.grad is not None:
            ...
            #print('grad of ', tag)
            logger.add_histogram(f"{model_name}/grads/{tag}", value.grad.cpu(), step)
            #added all below
            logger.add_histogram(f"{model_name}/weights/{tag}", value.data.cpu(), step)
            # print('valueshape', 'tag',value.cpu().shape, model_name, tag)
            if value.dim() ==4:
                filter_list.append(value)
                # for i in range(value.cpu().shape[0]):

                # for k in range(value.cpu().shape[1]):
                #   plt.imshow(value.cpu().detach().numpy()[0][k], cmap='gray')
                #   plt.show()

              # fig = full_fig(value.cpu()) #[0][0]).
              # logger.add_figure(f'{model_name}/{tag}',fig,step)
    # fig = full_fig2(filter_list[0])
    # plt.imshow(fig)
    # plt.show()
    return filter_list
